...
(defun sprites-dibuja (pixels posición ancho alto plano-camara dirección sprites zbuffer texturas)
  ...
  (loop with cam-minv = (minv (mat (vx2 plano-camara) (vx2 dirección)
     ...
     for sprite-screen-x fixnum = (the fixnum (truncate (* (/ ancho 2.0) (1+ (divseg (vx2 spos-trans)
                                                                                     (vy2 spos-trans))))))
     and sprite-alto single-float  = (abs (divseg alto (vy2 spos-trans)))
     ...
     do (loop for x single-float from (if (minusp x-ini) 0 x-ini) below (if (>= x-fin ancho) (1- ancho) x-fin)
           ...
           if (and (plusp (vy2 spos-trans))
                   (< (vy2 spos-trans) (aref zbuffer x-fix))
                   (plusp x) (< x ancho) )
